/**
 * Backend Google Apps Script Code templates for SHE. Academy
 * - GOOGLE_APPS_SCRIPT_CODE: Unified script handling WorkshopDates + Bookings + Contact + Feedback
 * - WORKSHOP_DATES_MANAGER_APPS_SCRIPT: Dedicated script for the WorkshopDates Google Sheet (Col A: Language, Col B: DatesJSON)
 */
...
function doGet(e) {
  try {
    ...
    // Default browser view: Interactive Workshop Dates Manager Dialog
...
export const WORKSHOP_DATES_MANAGER_APPS_SCRIPT = `/**
 * ============================================================================
 * SHE. ACADEMY - WORKSHOP DATES MANAGER (DEDICATED GOOGLE APPS SCRIPT)
 * For Google Sheet: "WorkshopDates"
 *   Row 1: Col A = "Language" | Col B = "DatesJSON"
 *   Row 2: Col A = "EN"       | Col B = ["...", "..."]  (English Website .com)
 *   Row 3: Col A = "NO"       | Col B = ["...", "..."]  (Norwegian Website .no)
 *   Row 4: Col A = "CZ"       | Col B = ["...", "..."]  (Czech Website .cz)
 *   Row 5: Col A = "SHE"      | Col B = ["...", "..."]  (SHE. Academy Sanctuary sheacademy.no)
 *
 * Missing tabs or headers are auto-created!
 * ============================================================================
 */

const SHEET_NAME = "WorkshopDates";
const SUPPORTED_LANGS = ["EN", "NO", "CZ", "SHE"];
const DEFAULT_ROWS = [
  { code: "EN", row: 2, name: "English (.com)" },
  { code: "NO", row: 3, name: "Norwegian (.no)" },
  { code: "CZ", row: 4, name: "Czech (.cz)" },
  { code: "SHE", row: 5, name: "SHE. Academy (sheacademy.no)" }
];

// 1. Helper: Get or automatically create "WorkshopDates" sheet and headers
function getOrCreateWorkshopDatesSheet(ss) {
  ...
  if (lastRow >= 2) {
    const colA = sheet.getRange(2, 1, lastRow - 1, 1).getValues();
    for (let i = 0; i < colA.length; i++) {
      const c = String(colA[i][0] || "").trim().toUpperCase();
      if (c) existing[c] = i + 2;
    }
  }

  DEFAULT_ROWS.forEach(function(item) {
    if (!existing[item.code]) {
      const targetRow = item.row;
      const cellVal = sheet.getRange(targetRow, 1).getValue();
      const insertRow = (!cellVal) ? targetRow : (sheet.getLastRow() + 1);
      sheet.getRange(insertRow, 1).setValue(item.code);
      if (!sheet.getRange(insertRow, 2).getValue()) {
        sheet.getRange(insertRow, 2).setValue("[]");
      }
      existing[item.code] = insertRow;
    }
  });
...
function getDatesData() {
  ...
  if (lastRow >= 2) {
    ...
    for (let i = 0; i < vals.length; i++) {
      const lang = String(vals[i][0] || "").trim().toUpperCase();
      const rawJson = vals[i][1];
      if (lang) {
        let parsed = [];
        if (typeof rawJson === "string" && rawJson.trim()) {
          try {
...
function doGet(e) {
  try {
    ...
    return HtmlService.createHtmlOutput(getManagerHtml())
      .setTitle("SHE. Academy - Workshop Dates Manager")
      .setXFrameOptionsMode(HtmlService.XFrameOptionsMode.ALLOWALL);
  } catch (error) {
    return ContentService.createTextOutput(JSON.stringify({
...
function doPost(e) {
  try {
    ...
    if (action === "sync_workshops") {
      ...
      return ContentService.createTextOutput(JSON.stringify({
        status: "success",
        message: "Synchronized " + entries.length + " workshops to [" + targets.join(", ") + "] in WorkshopDates tab",
        count: entries.length
      })).setMimeType(ContentService.MimeType.JSON);
    }
    ...
    if (action === "add_workshop") {
      ...
      return ContentService.createTextOutput(JSON.stringify({
        status: "success",
        message: "Workshop added to [" + targets.join(", ") + "] in WorkshopDates tab",
        entry: entry
      })).setMimeType(ContentService.MimeType.JSON);
    }
    ...
    if (action === "save_dates" || data.currentData) {
      ...
      return ContentService.createTextOutput(JSON.stringify({
        status: "success",
        message: "Successfully saved dates"
      })).setMimeType(ContentService.MimeType.JSON);
    }
    ...
    if (action === "init_tabs") {
      ...
      return ContentService.createTextOutput(JSON.stringify({
        status: "success",
        message: "WorkshopDates sheet initialized with Language & DatesJSON headers."
      })).setMimeType(ContentService.MimeType.JSON);
    }
...
// 6. Google Sheets Menu
function onOpen() {
  SpreadsheetApp.getUi()
    .createMenu("🌸 SHE. Academy")
    .addItem("Workshop Dates Manager UI", "showDialog")
    .addItem("Initialize WorkshopDates Sheet & Headers", "initSheet")
    .addToUi();
}

function showDialog() {
  const html = HtmlService.createHtmlOutput(getManagerHtml())
    .setWidth(780)
    .setHeight(650);
  SpreadsheetApp.getUi().showModalDialog(html, "🌸 Workshop Dates Manager (EN, NO, CZ, SHE)");
}
...
function initSheet() {
  ...
  SpreadsheetApp.getUi().alert(
    "Success",
    'Sheet "WorkshopDates" is initialized with headers [Language, DatesJSON] and rows: EN (Row 2), NO (Row 3), CZ (Row 4), SHE (Row 5).',
    SpreadsheetApp.getUi().ButtonSet.OK
  );
}

// 7. Interactive HTML Dialog UI
...
'      </div>' +
'      <div class="info-box">' +
'        Directly updates <code>DatesJSON</code> in the <strong>WorkshopDates</strong> sheet across all 4 websites. Missing rows or headers are auto-generated.' +
'      </div>' +
'      <div class="tabs" id="tabs" style="display:none;">' +
'        <div class="tab active" onclick="switchTab(\\\'EN\\\')">EN<span class="tab-label">.com (Row 2)</span></div>' +
'        <div class="tab" onclick="switchTab(\\\'NO\\\')">NO<span class="tab-label">.no (Row 3)</span></div>' +
