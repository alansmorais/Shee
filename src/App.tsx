...
export default function App() {
    ...
    if (targetId) {
      setTimeout(() => {
        ...
        if (el) {
          ...
          el.classList.add('ring-2', 'ring-accent-editorial', 'ring-offset-4');
          setTimeout(() => {
            el.classList.remove('ring-2', 'ring-accent-editorial', 'ring-offset-4');
          }, 2500);
        }
      }, 120);
    } else {
  ...
  return (
    <div id="she-app" className="min-h-screen flex flex-col bg-main text-main font-sans antialiased selection:bg-accent-editorial selection:text-white">
      ...
      {bannerNotice && (
        <div className="bg-accent-editorial text-white text-xs py-2.5 px-4 text-center relative flex items-center justify-center">
          <span>{bannerNotice}</span>
          <button
            type="button"
            onClick={() => setBannerNotice(null)}
            className="absolute right-4 text-white/80 hover:text-white font-bold text-sm cursor-pointer"
