...
export const AdminDashboard: React.FC<AdminDashboardProps> = ({
    ...
    const workshopData: Workshop = {
      ...
      facilitators:
        editingWorkshop.facilitators && editingWorkshop.facilitators.length > 0
          ...
          : ['Mgr. Daniela Torp'],
      status: editingWorkshop.status || 'open',
  ...
  const [tempAdminEmail, setTempAdminEmail] = useState(settings.adminEmail || 'contact@sheacademy.no');
                    ...
                    <tbody className="divide-y divide-line">
                      {workshopsList.map((ws) => (
                        <tr key={ws.id} className="hover:bg-alt/30 transition-colors">
                          ...
                          <td className="p-4">
                            <span
                              className={`px-2.5 py-0.5 rounded-full text-[11px] font-medium border inline-flex items-center space-x-1.5 ${
                                ws.status === 'sold_out'
                                  ? 'border-amber-500/30 bg-amber-500/10 text-amber-700 dark:text-amber-300'
                                  : ws.status === 'closed'
                                  ? 'border-line bg-alt text-muted-editorial'
                            ...
                            >
                              ...
                              />
                              <span>
                                {ws.status === 'sold_out' ? 'Sold Out' : ws.status === 'closed' ? 'Closed' : 'Open'}
                              </span>
                            </span>
                          </td>
        ...
        {activeTab === 'settings' && (
          <div className="space-y-8 fade-in max-w-4xl">
            ...
            <div className="p-6 rounded-2xl border border-line bg-card space-y-6">
              {!isDevAuthenticated ? (
                <div className="space-y-4">
                  ...
                  <div className="flex items-center space-x-3">
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        if (devAuthError) setDevAuthError(null);
                      }}
                    ...
                    <button
                      ...
                      onClick={() => {
                        if (devPasswordInput === settings.developerPassword) {
                          ...
                          setDevAuthError('Invalid Developer Password');
                        }
                      }}
                  ...
                  {devAuthError && (
                    <p className="text-xs text-red-600 dark:text-red-400 mt-1">{devAuthError}</p>
                  )}
                </div>
              ) : (
                <>
            ...
            <div className="p-6 rounded-2xl border border-line bg-card space-y-3">
              ...
              <button
                ...
                onClick={() => {
                  if (confirm('Reset database to initial pristine state?')) {
                    ...
                    onRefreshData();
                    setDbResetNotice('Database has been reset to pristine defaults.');
                    setTimeout(() => setDbResetNotice(null), 4000);
                  }
                }}
              ...
              {dbResetNotice && (
                <p className="text-xs text-emerald-600 dark:text-emerald-400 mt-2 font-medium">{dbResetNotice}</p>
              )}
            </div>
          </div>
        )}
          ...
          <div className="w-full max-w-2xl bg-card border border-line rounded-2xl p-6 sm:p-8 space-y-6 shadow-2xl my-8">
            <div className="flex items-center justify-between border-b border-line pb-4">
              <div>
                ...
                <p className="text-xs text-muted-editorial">
                  Configure details, dates, facilitator faculty, and registration status for public display.
                </p>
              </div>
              <button
                type="button"
            ...
            <form onSubmit={handleSaveWorkshop} className="space-y-4 text-xs">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <select
                    value={editingWorkshop.status || 'open'}
                    onChange={(e) =>
                      setEditingWorkshop({
                        ...editingWorkshop,
                        status: e.target.value as 'open' | 'closed' | 'sold_out',
                      })
                    }
                    className="w-full px-3 py-2 rounded-xl border border-line bg-main text-main text-xs focus:outline-none focus:border-accent-editorial cursor-pointer"
                  >
